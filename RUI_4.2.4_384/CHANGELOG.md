...
### Added
- support RAK3172-F which has external SPI flash  
- Add AT+EXTFLSSH for checking external flash ID for production  
- Add RUI API for external flash  
  Add `api.system.extflash.get(uint32_t addr, uint8_t *buf, uint32_t len)`  
  Add `api.system.extflash.set(uint32_t addr, const uint8_t *data, uint32_t len)`  
  Add `api.system.extflash.erase(uint32_t addr, uint32_t size)`  
  Add `api.system.extflash.info(EXTFLASH_INFO_t *info)`  
...
### Fixed
- RUI-1135: [RUI3 V4.2.1][RAK3172] Switching between confirmed and unconfirmed uplink in api.lorawan.send fails  
- RUI-1137: [4.2.1]After removing multicast session, unicast is not working anymore  
- RUI-1138: Saving fCnt on stack V1.0.4 not necessary when device is using OTAA.  
- RUI-1146: [RAK4631] [RUI3 V4.2.1] analogWrite crashes RUI3.
